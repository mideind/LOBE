...
def download_recording(id):
    ...
    try:
        ...
        app.logger.error("Error downloading a recording : {}\n{}".format(error, traceback.format_exc()))
...
def record_session(collection_id):
    ...
    if not user_id:
        ...
        return redirect(url_for("collection.collection_detail", id=collection_id))
...
def analyze_audio():
    ...
    body = {
        "analysis": message,
        "segment": {"start": float(segment_times[0]), "end": float(segment_times[1])},
    }
