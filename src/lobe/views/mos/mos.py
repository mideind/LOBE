...
def mos_list():
    ...
    return render_template("mos_list.jinja", mos_list=mos_list, collections=collections, section="mos")
...
def mos_collection(id):
    ...
    mos_list = (
        Mos.query.filter(Mos.collection_id == id)
        .order_by(
            resolve_order(
                Mos,
                request.args.get("sort_by", default="created_at"),
            ...
            if form.is_g2p.data:
                ...
                with ZipFile(zip_file, "r") as zip:
                    ...
                    if len(successfully_uploaded) > 0:
                        flash(
                            "Tókst að hlaða upp {} setningum.".format(len(successfully_uploaded)),
                            category="success",
                        )
                    else:
                        flash(
    ...
    if request.method == "POST":
        if form.validate():
            try:
                ...
                return redirect(url_for("mos.take_mos_test", mos_uuid=mos_uuid))
            return redirect(url_for("mos.mos_test", id=mos.id, uuid=new_user.uuid))
...
def stream_MOS_zip(id):
    ...
    return Response(
        generator,
        mimetype="text/plain",
        headers={"Content-Disposition": "attachment;filename={}_tokens.txt".format(mos.printable_id)},
    )
...
def stream_MOS_index_demo():
    ...
    try:
        return send_from_directory(other_dir, "synidaemi_mos.zip", as_attachment=True)
...
def post_mos_rating(id):
    ...
    if current_user.is_anonymous:
        return Response(url_for("mos.mos_done", id=mos_id), status=200)
    else:
...
def download_custom_recording(id):
    ...
    try:
        return send_from_directory(custom_recording.get_directory(), custom_recording.fname, as_attachment=True)
    except Exception as error:
