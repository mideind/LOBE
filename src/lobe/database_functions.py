...
def create_tokens(collection_id, files, is_g2p):
    ...
    if num_errors > 0:
        flash(
            f"{num_errors} villur komu upp, fyrsta villan í" + f"{error_file} í línu {error_line}",
            category="danger",
        )
            ...
            if row[0] and (len(row) == 3 or len(row) == 5 or len(row) == 6):
                ...
                for zip_info in zip.infolist():
                    ...
                    if zip_info.filename == row[0]:
                        custom_token_name = "{}_m{:09d}".format(zip_name, id)
                        ...
                        file_id = "{}_s{:09d}_m{:09d}".format(
                            os.path.splitext(os.path.basename(zip_info.filename))[0],
                            custom_recording.id,
                            id,
                        )
                        fname = secure_filename(f"{file_id}.webm")
                        path = os.path.join(app.config["CUSTOM_RECORDING_DIR"], str(id), fname)
            ...
            if row[0] and len(row) >= 2:
                for zip_info in zip.infolist():
                    ...
                    if zip_info.filename == row[0]:
                        ...
                        zip_info.filename = "{}.wav".format(os.path.splitext(os.path.basename(recording.fname))[0])
                        export_to_path = secure_filename(
                            "{}.webm".format(os.path.splitext(os.path.basename(row[0]))[0])
                        )
...
def is_valid_info(data):
    if "collection_info" in data and "text_info" in data and "recording_info" in data and "other" in data:
        if (
            "text" in data["text_info"]
            and "session_id" in data["collection_info"]
            and "recording_fname" in data["recording_info"]
            and "text_marked_bad" in data["other"]
            and "recording_marked_bad" in data["other"]
            and "duration" in data["recording_info"]
        ):
            return True
    return False
            ...
            if is_valid_info(info[key]):
                for zip_info in zip.infolist():
                    ...
                    if zip_info.filename == info_filename:
                        ...
                        zip_info.filename = "{}.wav".format(os.path.splitext(os.path.basename(recording.fname))[0])
                        export_to_path = secure_filename(
                            "{}.webm".format(os.path.splitext(os.path.basename(info_filename))[0])
                        )
                        export_to_path = os.path.join(dirs[0], export_to_path)
                        zip.extract(zip_info, dirs[3])
                        sound = AudioSegment.from_wav(recording.wav_path)
...
def delete_rating_if_exists(mos_instance_id, user_id):
    rating = (
        MosRating.query.filter(MosRating.mos_instance_id == mos_instance_id).filter(MosRating.user_id == user_id).all()
    )
    exists = False
    for r in rating:
        exists = True
    ...
    for day, sessions in days.items():
        ...
        day_info[day] = {
            ...
            "est_work_time": datetime.timedelta(
                seconds=math.ceil((sessions[-1].created_at - sessions[0].get_start_time).total_seconds())
            ),
            "session_duration": datetime.timedelta(seconds=int(sum(s.duration for s in sessions))),
        }

    total_est_work_time = sum((i["est_work_time"] for _, i in day_info.items()), datetime.timedelta(0))
    total_session_duration = sum((i["session_duration"] for _, i in day_info.items()), datetime.timedelta(0))
...
def get_admins():
    return [u for u in User.query.all() if u.is_admin()]
