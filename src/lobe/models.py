...
class Collection(BaseModel, db.Model):
    ...
    def get_complete_ratio(self, as_percent=False):
        if self.num_tokens == 0 or self.number_of_users == 0:
            ...
            if not self.is_multi_speaker:
                ...
                ratio = (self.number_of_recordings) / (self.num_tokens * self.number_of_users)
        if as_percent:
    ...
    def estimate_hours(self):
        ...
        if not self.is_multi_speaker:
            return round(
                (self.num_tokens - self.num_nonrecorded_tokens) * ESTIMATED_AVERAGE_RECORD_LENGTH / 3600,
                1,
            )
        else:
    ...
    def get_user_number_of_recordings(self, user_id):
        ...
        if user_id in user_ids:
            recordings = (
                Recording.query.join(Recording.token)
                .filter(Token.collection_id == self.id, Recording.user_id == user_id)
                .count()
            )
            return recordings
        return False
    ...
    def get_users_number_of_recordings(self, user_ids):
        recordings = (
            Recording.query.join(Recording.token)
            .filter(Token.collection_id == self.id, Recording.user_id.in_(user_ids))
            .with_entities(Recording.user_id, func.count(Recording.id))
    ...
    def number_of_recordings(self):
        recordings = Recording.query.join(Recording.token).filter(Token.collection_id == self.id).count()
        return recordings
    ...
    def user_ids(self):
        if not self.is_multi_speaker:
            ...
            user_ids = (
                Recording.query.join(Recording.token)
                .filter(Token.collection_id == self.id, Token.num_recordings > 0)
                .with_entities(Recording.user_id)
                .distinct(Recording.user_id)
                .all()
            )
...
class Configuration(BaseModel, db.Model):
    ...
    def media_constraints(self):
        ...
        if self.has_video:
            constraints["video"] = {"width": self.video_w, "height": self.video_h}
        return constraints
...
class Token(BaseModel, db.Model):
    ...
    def set_path(self):
        self.fname = secure_filename("{}_{:09d}.token".format(os.path.splitext(self.original_fname)[0], self.id))
    ...
    def update_numbers(self):
        self.num_recordings = Recording.query.filter(Recording.token_id == self.id).count()
...
class CustomToken(BaseModel, db.Model):
    ...
    def set_path(self):
        self.fname = secure_filename("{}_u{:09d}.token".format(os.path.splitext(self.original_fname)[0], self.id))
        self.path = os.path.join(app.config["CUSTOM_TOKEN_DIR"], str(self.mos_id), self.fname)
    ...
    def get_configured_path(self):
        ...
        if self.copied_token:
            path = os.path.join(app.config["TOKEN_DIR"], str(self.collection_id), self.fname)
            return path
        path = os.path.join(app.config["CUSTOM_TOKEN_DIR"], str(self.mos_id), self.fname)
        return path
...
class Recording(BaseModel, db.Model):
    ...
    priority_session_id = db.Column(db.Integer, db.ForeignKey("PrioritySession.id"))
    ...
    def get_configured_path(self):
        ...
        w.r.t. the current TOKEN_DIR environment variable
        """
        return os.path.join(app.config["RECORD_DIR"], str(self.token.collection_id), self.fname)
...
class CustomRecording(BaseModel, db.Model):
    ...
    def get_configured_path(self):
        ...
        if self.copied_recording:
            path = os.path.join(app.config["RECORD_DIR"], str(self.token.collection_id), self.fname)
            return path
        path = os.path.join(
...
class Verification(BaseModel, db.Model):
    ...
    def recording_is_good(self):
        return not any(
            [
                self.recording_has_glitch,
                self.recording_has_wrong_wording,
                self.volume_is_high,
                self.volume_is_low,
            ]
        )
...
class Mos(BaseModel, db.Model):
    ...
    num_samples = db.Column(db.Integer, default=0, info={"label": "Fjöldi setninga"})
        ...
        for row in latinSquareRows:
            ...
            while len(configuration) < len(list(utterances)):
                configuration.extend(
                    [
                        x
                        for x in self.mos_objects
                        if (
                            x.voice_idx == row[len(configuration) % len(row)]
                            and x.utterance_idx == utterances[len(configuration)]
                        )
                    ]
                )
...
class MosInstance(BaseModel, db.Model):
    ...
    custom_token = db.relationship(
        "CustomToken",
        lazy="joined",
        backref=db.backref("mosInstance", uselist=False),
        uselist=False,
        cascade="all, delete, delete-orphan",
    )
    ...
    selected = db.Column(db.Boolean, default=False, info={"label": "Hafa upptoku"})
...
class MosRating(BaseModel, db.Model):
    ...
    __table_args__ = (db.UniqueConstraint("mos_instance_id", "user_id"),)
    id = db.Column(db.Integer, primary_key=True, nullable=False, autoincrement=True)
    rating = db.Column(
        db.Integer,
        default=0,
